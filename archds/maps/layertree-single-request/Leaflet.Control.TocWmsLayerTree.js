L.Control.TocWmsLayerTree = L.Control.LayerTree.extend({
  ...
  _tocLayerToLayerTreeLayer: function (tocLayer, indexWmsLayerNames) {
    ...
    var layerTreeLayer = new L.Control.LayerTree.LayerNode(tocLayer.name, {
  ...
  wmsLayersList: function (mapFunction) {
    var wmsLayersFullNamesList = this._layerTree.accumulate(function (node) {
      return node.options.wmsLayerFullName
    })
    var wmsLayersFullNamesString = wmsLayersFullNamesList.join(",")
    var layersWmsParams = L.extend({layers: wmsLayersFullNamesString}, this.options.wmsParams)

    var leafletLayer = L.tileLayer.wms(this.options.wmsUrl, layersWmsParams)
