...
archds.maps.control.FullScreen = function(opt_options) {
  ...
  var button = goog.dom.createDom(goog.dom.TagName.BUTTON, {
    'class': this.cssClassName_ + '-' + goog.dom.fullscreen.isFullScreen() + " button",
    'type': 'button',
    'title': tipLabel
  }, this.labelNode_);
...
archds.maps.control.FullScreen.prototype.handleFullScreen_ = function() {
  ...
  this.bounds = view.calculateExtent(map.getSize())
...
archds.maps.control.FullScreen.prototype.handleFullScreenChange_ = function() {
  ...
  if (!goog.isNull(map)) {
    map.updateSize();
    view.fit(this.bounds, map.getSize(), {constrainResolution: false})
  }
};
