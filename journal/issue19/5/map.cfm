...
<body>
  ...
  var footnote = false, ce = {}, namespace = false
  ...
  for (i in query_parts) {
    ...
    switch (query_part_kv[0]) {
      case "footnote":
        ...
        break
      case "namespace":
        namespace = query_part_kv[1]
        break
      case "ceminlng":
      case "ceminlat":
      case "cemaxlng":
      case "cemaxlat":
        ce[query_part_kv[0]] = parseFloat(query_part_kv[1])
        break
    }
  }
  ...
  if (footnote) {
    ...
    map_load(footnote, ce, namespace)
  } else {
  ...
  function map_load(toc_footnote, ce, namespace) {
    ...
    toc_url = "/journal/issue19/5/archds-leaflet-mapping/tocs/" + toc_footnote + ".toc"
  ...
  function map_display(ce, toc) {
    var ce_bounds = [
      [ce["ceminlat"], ce["ceminlng"]],
      [ce["ceminlat"], ce["cemaxlng"]],
      [ce["cemaxlat"], ce["cemaxlng"]],
      [ce["cemaxlat"], ce["ceminlng"]]
    ]
    ...
    var minimapWmsLayer = L.tileLayer.wms(geoserver_url + "/wms", {
      layers: namespace + ":13," + namespace + ":14," + namespace + ":15," + namespace + ":16," + namespace + ":17," + namespace + ":18," + namespace + ":19," + namespace + ":20," + namespace + ":21," + namespace + ":22," + namespace + ":23",
      format: "image/png",
      transparent: true,
      crs: L.CRS.EPSG3857
    })
